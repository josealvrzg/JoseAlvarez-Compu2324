...
lamb = 0.3                          # Altura potencial
B = 6                               # Numero de barreras de potencial

T = 2700*B                            # Iteraciones temporales
...
j = np.array(range(0,N*B))            # Reticulo espacial
alpha = np.zeros(N*B-1,dtype=complex) # alpha 
beta = np.zeros(N*B-1,dtype=complex)  # beta
gamma = np.zeros(N*B-1,dtype=complex) # gamma
ji = np.zeros(N*B,dtype=complex)      # ji
Vprim = np.zeros(N*B)                 # Potencial
...
for b in range(0,B):
    for k in range(N*b,N*(b+1)):
        if  2/5*N+b*N <= k <= 3/5*N+b*N:                            # Añade valores solo en el intervalo seleccionado
...
def calculos(phi,beta,ji):
    ...
    for k in range(B*N-2,0,-1):                           # Calculo beta para un instante n                       
        beta[k-1] = gamma[k]*(4*i*phi[k]/sprim-beta[k])

    ji[0]=ji[B*N-1]=0                                     # Calculo ji para un instante n
...
def probderecha(phi):
    ...
    Pd = np.sum(np.abs(phi[3*N//5+N*(B-1):])**2)/np.sum(np.abs(phi)**2) # Dividido entre la norma para normalizar
    return Pd
...
while lanz<m:
    ...
    phi = np.array(np.exp(i*k0prim*j)*np.exp(-(j*h-x0)**2/(2*sigma**2)),
                dtype=complex)
    phi[0]=phi[N*B-1]=0                                       # Condiciones de contorno

    ## alpha y gamma (solo llega hasta N-1 en j)
    A0 = -2+2*i/sprim-Vprim
    for k in range(N*B-2,0,-1):                               #range(inicio, final, paso) decreciente
        alpha[k-1] = -1/(A0[k]+alpha[k])
    for k in range(N*B-1,0,-1):
...
plt.title('Función de onda en el momento de medida, con B=6')
plt.ylabel(r'$\Psi(x)$')
plt.xlabel('x')
plt.show()


print(r"El coficiente de Transmisión K=", K ,r"para un valor de B=",B)
